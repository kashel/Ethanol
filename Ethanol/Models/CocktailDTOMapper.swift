...
struct CocktailDTOMapper {
  func map(dto: CocktailDTO) -> Cocktail {
    ...
    return Cocktail(name: dto.name, description: dto.description, directions: dto.directions, imageName: imageName, ingredients: Set(dto.ingredients.map(mapIngredient)))
  }
  ...
  private func mapIngredient(dto: CocktailDTO.IngredientDTO) -> Cocktail.Ingredient {
    ...
    return Cocktail.Ingredient(id: dto.ingredient, unit: unit, amount: amount)
  }
}
